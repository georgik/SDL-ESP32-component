	...
	do
	{
		if (episodeAvail[x-1])
		{
			...
			for (z = 0; z < 3; z++)
			{
				...
				sprintf(scoretemp, "~#%d:~ %d", z + 1, (int)saveFiles[temp+z].highScore1);
			...
			for (z = 0; z < 3; z++)
			{
				...
				sprintf(scoretemp, "~#%d:~ %d", z + 1, (int)saveFiles[temp+z].highScore1); /* Not .highScore2 for some reason */
...
void JE_gammaCorrect( Palette *colorBuffer, JE_byte gamma )
{
	...
	for (x = 0; x < 256; x++)
	{
		JE_gammaCorrect_func((JE_byte*)&(*colorBuffer)[x].r, r);
		JE_gammaCorrect_func((JE_byte*)&(*colorBuffer)[x].g, r);
		JE_gammaCorrect_func((JE_byte*)&(*colorBuffer)[x].b, r);
	}
}
		...
		if (slot < slot_limit)
		{
...

			{
				...
				for (int i = first_slot; i < slot_limit; ++i)
				{
					if (i != slot)
					{
						sprintf(buffer, "~#%d:~  %d", (i - first_slot + 1), (int)saveFiles[i].highScore1);
				...
				sprintf(buffer, "~#%d:~  %d", (slot - first_slot + 1), (int)saveFiles[slot].highScore1);
